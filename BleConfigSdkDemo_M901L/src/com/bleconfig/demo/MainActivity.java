...
public class MainActivity extends Activity implements OnClickListener{
	...
	private View vDeviceId, vWifi, vAPN;
	...
	private boolean showWifiInput = false;
...
//		SdkLog.setLogDir(dir);
	...
	private void findView() {
		...
		vWifi = findViewById(R.id.layout_wifi);
		...
		vAPN = findViewById(R.id.tv_set_apn);
	}
...
//		port = mSetting.getInt("port", 3010);
		...
		port = mSetting.getInt("port", 0);//29020
...
//		etPort.setSelection(etPort.length());
	...
	protected void onPause() {
		...
		if(!TextUtils.isEmpty(strPort)) {
			editor.putInt("port", Integer.valueOf(strPort));
		}
		editor.commit();
	}
	
	private Runnable clearCounterTask = new Runnable() {
		@Override
		public void run() {
			// TODO Auto-generated method stub
			clickCount = 0;
	...
	public void onClick(View v) {
		...
		if(v == vAPN) {
			...
			handler.removeCallbacks(clearCounterTask);
			clickCount++;
			if(clickCount >= 10) {
				clickCount = 0;
				showWifiInput = !showWifiInput;
				vWifi.setVisibility(showWifiInput ? View.VISIBLE : View.GONE);
			}else {
				handler.postDelayed(clearCounterTask, 3000);
			}
		}else if(v == etSsid) {
			...
			loadingDialog.show();
			wifiConfigHelper.bleWiFiConfig(device.getDeviceType().getType(), device.getAddress(), ip, port, showWifiInput?ssidRaw:null, showWifiInput?pwd:null, callback);
		}
	}
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if(cd.getStatus() == StatusCode.DISCONNECT) {
						showDisconnectDialog(MainActivity.this);
					}else {
...
//		}
	...
	private void showDisconnectDialog(Context context) {
        ...
        int msgRes = R.string.reminder_connect_ble;
        tvTips.setText(msgRes);
        btn.setText(R.string.btn_ok);
